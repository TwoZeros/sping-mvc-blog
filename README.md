...
Первая задача достаточно простая: необходимо смигрировать приложение на сервлетах, написанное в предыдущих ДЗ на Spring Web MVC с Embed Tomcat.

### Задача

Создать новый проект на базе Spring MVC и Embed Tomcat и перенести реализованную в предыдущих ДЗ функциональность.

Контроллер должен выглядеть именно так, как в лекции:
```java
@RestController
...
Вся функциональность (CRUD), реализованная до этого, должна по-прежнему работать.

### Результат

GitHub проект реализованный согласно ТЗ, ссылка на него отправлена в личном кабинете студента на сайте [netology.ru](https://netology.ru).
...
Создать Pull Request с описанной функциональностью к проекту из первой задачи (Migration).
